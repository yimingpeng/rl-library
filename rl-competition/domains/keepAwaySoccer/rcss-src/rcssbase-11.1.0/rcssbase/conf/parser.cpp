...
namespace rcss
{
    namespace conf
    {
		...
		static
		std::string
		cleanString( const char* begin, const char* end )
		{
		...
		static
		void
		setPath( const char* begin, const char* end )
		{
		...
		static
		void
		addPath( const char* begin, const char* end )
		{
			std::string spath = cleanString( begin, end );
			boost::filesystem::path path;
			try
			...
			if( argc > 1 )
      {
		...
		void
		Parser::countNewLines( const char* begin, const char* end )
		{
			int count = 0;
        ...
        bool
        Parser::include( const char* begin, const char* end )
        {
		...
		void
		Parser::load( const char* begin, const char* end )
		{
			lib::Loader loader;
		...
		bool
		Parser::setParamName( const char* begin, const char* end )
		{
		...
		void
		Parser::appendParamName( const char* begin, const char* end )
		{
		...
		Parser::boostParse()
		{
			...
			boost::spirit::rule<> junk_p = ( 
				...
				| comment_p
				[ 
					boost::bind( &Parser::countNewLines, this, _1, _2 ) 
					] 
				| newline_p
				[ 
					boost::bind( &Parser::countNewLines, this, _1, _2 ) 
			...
			boost::spirit::rule<> includerule_p = ( 
				...
				>> expect_string( 
					string_p[
						boost::bind( &Parser::include, this, _1, _2 ) 
					])
				);
			...
			boost::spirit::rule<> loadrule_p = ( 
				...
				>> expect_string( 
					string_p[
						boost::bind( &Parser::load, this, _1, _2 ) 
					])
				);


			boost::spirit::rule<> pathrule_p = ( 
				( boost::spirit::as_lower_d[ "setpath" ]
				  >> ignore_p
				  >> expect_assign( assign_p )
				  >> ignore_p
			...
			boost::spirit::rule<> param_name_p = (
				(simple_str_p - flag_p)[
					boost::bind( &Parser::setParamName,
								 this,
								 _1, _2 )  
				 ]
			...
			boost::spirit::rule<> true_p = ( 
				boost::spirit::as_lower_d[ "on" ]
				| boost::spirit::as_lower_d[ "true" ] 
				);
			
			boost::spirit::rule<> false_p = ( 
				boost::spirit::as_lower_d[ "off" ]
			...
			boost::spirit::rule<> param_p = ( 
				...
				>> expect_value( true_p[
								 ...
								 | boost::spirit::strict_real_p[
									 boost::bind( &Parser::buildReal,
												  this,
												  _1 )
								 ]
								 |
								 boost::spirit::int_p[
									 boost::bind( &Parser::buildInt,
												  this,
												  _1 )
								 ]
								 | string_p[
									 boost::bind( &Parser::buildString,
												  this,
												  _1, _2 )
								 ]) 
				);
			...
			boost::spirit::rule<> data_p = ( 
			    boost::spirit::as_lower_d[ "help" ][
					boost::bind( &Parser::requestGenericHelp,
								 this )
				]
				| ( module_name_p
					>> delim_p 
			...
			boost::spirit::rule<> item_p = ( 
				junk_p 
				| ( !minus_p
					>> !minus_p
					>> data_p )
				);
			boost::spirit::rule<> input_p = conf_guard( *item_p )[
				boost::bind( &ParseErrorHandler::parseError,
							 this, _1, _2 )
			];
