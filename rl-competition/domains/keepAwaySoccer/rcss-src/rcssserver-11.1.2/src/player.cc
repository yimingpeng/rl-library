...
Player::Player( Team *tm,
    ...
    : MPObject( stad,
      ...
      M_team( tm ),
      M_unum( number ),
      M_stamina( ServerParam::instance().stamina_max ),
      M_recovery( ServerParam::instance().recover_init ),
      //M_effort( 1.0 ),
      ...
      M_parser( *this )
{
    ...
    M_weight = ServerParam::instance().pweight;
    M_max_speed = ServerParam::instance().pspeed_max ;
    M_max_accel = ServerParam::instance().paccel_max;

    M_pos.x = -( unum() * 3 * tm->side() );
    M_pos.y = -PITCH_WIDTH/2.0 - 3.0;

    // pfr 8/14/00: for RC2000 evaluation
    M_kick_rand = ServerParam::instance().kick_rand;
...
Player::turn_neck( double moment)
{
    if ( ! M_turn_neck_done )
    {
        M_angle_neck = NormalizeNeckAngle( angleNeckCommitted() + NormalizeNeckMoment( moment ) );
        ++M_turn_neck_count;
...
Player::kick( double power, double dir )
{
    if ( ! M_command_done )
    {
        ...
        // pfr 8/14/00: for RC2000 evaluation
        // add noise to kick
        Value maxrnd = M_kick_rand * power / ServerParam::instance().maxp;
...
//                           PVector( PENALTY_AREA_LENGTH, PENALTY_AREA_WIDTH ) ) ;
        ...
        if ( ! catchable.inArea( rotated_pos )
             || drand( 0, 1 ) >= ServerParam::instance().catch_prob )
        {
            alive |= CATCH_FAULT;
            return ;
        }

        M_goalie_catch_ban = ServerParam::instance().cban_cycle;
        M_goalie_moves_since_catch = 0; // reset the number of times the goalie moved
        {
            ...
            mag -= ServerParam::instance().bsize + M_player_type->playerSize();
    ...
    if ( ! M_command_done )
    {
                  ...
                  && M_stadium->ballCatcher() == this )
        {
            if ( ServerParam::instance().goalie_max_moves < 0
                 || M_goalie_moves_since_catch < ServerParam::instance().goalie_max_moves )
            {
                M_pos.x = x * team()->side();
                M_pos.y = y * team()->side();
...
Player::setSenders()
{
    ...
    if( ! rcss::BodySenderPlayer::factory().getCreator( body_cre,
                                                        (int)version() ) )
        return false;
    setBodySender( body_cre( body_params ) );
    ...
    if( ! rcss::VisualSenderPlayer::factory().getCreator( vis_cre,
                                                          (int)version() ) )
        return false;
    setVisualSender( vis_cre( visual_params ) );
    ...
    setInitSender( init_cre( init_params ) );
    ...
    if( ! rcss::FullStateSenderPlayer::factory().getCreator( full_cre,
                                                             (int)version() ) )
        return false;
    setFullStateSender( full_cre( fs_params ) );
...
{
    if ( M_stamina <= ( ServerParam::instance().recover_dthr
                        * ServerParam::instance().stamina_max ) )
    {
        if ( M_recovery > ServerParam::instance().recover_min )
            M_recovery -= ServerParam::instance().recover_dec;
        if ( M_recovery < ServerParam::instance().recover_min )
            M_recovery = ServerParam::instance().recover_min;
    }

    if ( M_stamina <= ( ServerParam::instance().effort_dthr
                        * ServerParam::instance().stamina_max ) )
    {
        if ( M_effort > M_player_type->effortMin () )
            M_effort -= ServerParam::instance().effort_dec;
        if ( M_effort < M_player_type->effortMin () )
            M_effort = M_player_type->effortMin();
    }

    if ( M_stamina >= ( ServerParam::instance().effort_ithr
                        * ServerParam::instance().stamina_max ) )
    {
        if ( M_effort < M_player_type->effortMax() )
        {
            M_effort += ServerParam::instance().effort_inc;
            if ( M_effort > M_player_type->effortMax () )
                M_effort = M_player_type->effortMax ();
        }
    }
    ...
    if ( M_stamina > ServerParam::instance().stamina_max )
        M_stamina = ServerParam::instance().stamina_max;
}
