...
# are preserved.
#
# Desc: Adds support cvs targets within your Makefile.  
#
#       Branching and releasing relies on you using the following
#       version format:
#         MAJOR.MINOR.POINT
#       where MAJOR is the major version number, MINOR is the minor
#       version number and POINT is the point release number.
#
#       Targets:
#		* update
...
#
#		* commitlog
#			Generates a ChangeLog template with the modifed,
#			added and removed files and opens it up for editing.
#  			This is not normally used directly, but is instead
#			called by commit.  It is usefull however if you want
#			to document the work you have done, but don't want to
#   			commit just yet.
#
#		* commit
...
#			creates a release for the current version,
#			increaments the point release number and checks out
#			the release into a new direcory
#
#		* quick-release
...
# 
#	* Now checks for awk instead of gawk
#
AC_DEFUN([AX_CVS],
[
...
if test "x$AX_USING_CVS" != "xno"; then
   if test "x$AWK" = "x"; then
       if test "x$AX_USING_CVS" = "x"; then
	    ...
	    AC_MSG_ERROR([CVS support cannot be enabled: awk could not be found])
	fi
    fi
fi
...
branch-major:
	...
	$AWK -f ax_cvs_rel.awk -v change=1 \"${AX_DOLLAR}(top_srcdir)/configure.ac\" > configure.tmp; \\
...
do_tag:
	...
	REL_DIR=\"\$(top_builddir)/release/$PACKAGE\"; \\
	mkdir -p "\$\$REL_DIR"; \\
	(cd "\$\$REL_DIR"; $CVS -z9 -d$CVSROOT co -r ${AX_DOLLAR}${AX_DOLLAR}tag -d \"$VERSION\" $PACKAGE; ); \\
	echo \"The release is now available in the \"\$\$REL_DIR/$VERSION\" directory\"; \\
	echo \"\"

inc_rel: 
	@$AWK -f ax_cvs_rel.awk -v change=3 \"\$(top_srcdir)/configure.ac\" > configure.tmp;
