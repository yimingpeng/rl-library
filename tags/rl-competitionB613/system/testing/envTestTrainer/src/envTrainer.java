...
public class envTrainer{
    private final int numEnvs = 3;
    ...
    //You should write a bit of code to look at command line args to see what you want to run I guess.
        ...
        while(true){
            ...
            if(rand == 0){
                envTrainerHelper.loadMountainCar(0);
            }
            else if(rand ==1){
                envTrainerHelper.loadTetris(0);
            }
            else if(rand == 2){
                envTrainerHelper.loadHelicopter();
            }
	...
	public static void runMountainCarExperiment(){
		for(int whichParamSet=0;whichParamSet<10;whichParamSet++){
			envTrainerHelper.loadMountainCar(whichParamSet);
			runExperiment();
		}
	}
