...
namespace rcss
{
  namespace conf
  {
	  ...
	  public:
		  ...
		  RCSSBASE_API
          bool
          parse( const boost::filesystem::path& name );

		  /// for parsing a file and creating it if it doesn't exist
		  RCSSBASE_API
          bool
          parseCreateConf( const boost::filesystem::path& conf_name,
	  ...
	  private:
		  ...
		  void
		  countNewLines( const char* begin, const char* end );
              
		  bool
		  include( const char* begin, const char* end );

		  void
		  load( const char* begin, const char* end );
			  
		  bool
		  setParamName( const char* begin, const char* end );

		  void
		  appendParamName( const char* begin, const char* end );

		  bool
		  buildBool( bool value );
		  
		  bool
		  buildInt( int value );
		  
		  bool
		  buildReal( double value );
		  
		  bool
		  buildString( const char* begin, const char* end );
