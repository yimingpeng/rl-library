  ...
  *maxs = (double*)malloc(sizeof(double)*(*dim));
  ...
  for (i = 0; i < (*dim)-1; i++)
  {
	...
	if(strncmp(&((*types)[i]), "i", 1) == 0) /*if it's an int*/
		...
		printf("\nError: Variable type not of type int or float. GRRR1!! Variable of type %c Exiting... \n\n", (*types)[i]);
		exit(0);
	}		
  *ts = *ts + characters_read; /*move along the string*/
  } 
...
if(strncmp(&((*types)[i]), "i", 1) == 0)
	...
	printf("\nError: Variable type not of type int or float. GRRR2!! Variable of type %c Exiting... \n\n", (*types)[i]);
	exit(0);
}	
