...
client_dir=$keepaway_dir/player_network2   # directory containing player binary
...
launch_trainer_monitor=1         # should I launch trainer monitor on startup?
...
proc_name=$date-$machine-learning
...
server_opts="$s::port=$port $s::coach_port=$coach_port $s::olcoach_port=$olcoach_port $s::half_time=-1 $s::forbid_kick_off_offside=0 $s::use_offside=0 $s::stamina_inc_max=3500 $s::synch_mode=$synch_mode $s::synch_offset=80 $keepaway_opts $log_opts $vision_opts $s::coach=$use_trainer $s::slow_down_factor=1"
...
if (( $save_weights && ( 0 || $taker_learn ) )); then
