...
MPObject::wind()
{
    ...
    if ( M_stadium->weather().wind_rand < EPS )
    {
...
//                     (M_weight * WIND_WEIGHT));
    ...
    return PVector( M_vel.r() * ( w.wind_vector.x +
                                  drand( - w.wind_rand, w.wind_rand) ) /
                    (M_weight * WIND_WEIGHT), M_vel.r() * ( w.wind_vector.y +
                                                            drand( - w.wind_rand,
                                                                   + w.wind_rand ) ) /
                    (M_weight * WIND_WEIGHT));
}
...
// const
// PObject*
// PObjectTable::AssignObject( const PObject * obj )
// {
 ...
 *===================================================================
 */
MPObjectTable::MPObjectTable( Stadium & stadium )
    : M_stadium( stadium )
{
...
// std::ostream&
// operator<< ( std::ostream& o, const MPObjectTable& tbl )
// {
...
MPObject *
MPObjectTable::append( MPObject * obj )
{
    if ( obj )
    {
        M_objects.push_back( obj );
...
void
MPObjectTable::_inc( const PlayMode & playmode )
{
     std::random_shuffle( M_objects.begin(), M_objects.end(),
                          irand ); //rcss::random::UniformRNG::instance() );
     const Cont::iterator end = M_objects.end();
     for ( Cont::iterator it = M_objects.begin();
           it != end;
           ++it )
     {
         if ( (*it)->isEnable() )
...
void
MPObjectTable::collisions( const PlayMode & playmode )
{
    bool col = false;
    int max_loop = 10;

    const std::size_t SIZE = M_objects.size();
    const Cont::iterator end = M_objects.end();

    do
    {
        col = false;
        /*        std::cout << "max = " << max_loop << std::endl; */
        for ( Cont::iterator it = M_objects.begin();
              it != end;
              ++it )
        {
            (*it)->post_col_pos = PVector( 0, 0 );
            (*it)->col_count = 0;
        }
        ...
        for ( std::size_t i = 0; i < SIZE - 1; ++i )
        {
            ...
            {
                if ( M_stadium.detectCollision( M_objects[i], M_objects[j] ) )
                {
                    col = true;
                    calcCollPos( M_objects[i], M_objects[j], playmode );
                }
            }
        }
...
void
MPObjectTable::calcCollPos( MPObject * a,
                            MPObject * b,
                            const PlayMode playmode )
{
    ...
    if ( playmode != PM_PlayOn
         && a->getObjectType() == PObject::OT_BALL )
    {
        apos = a->pos();
        PVector a2b;
        if ( a->pos() == b->pos() )
        {
            Angle b_ang = drand( -M_PI, M_PI );
            a2b = PVector::fromPolar( a->size() + b->size(), b_ang );
        }
        else
        {
            a2b = b->pos() - a->pos();
            a2b.normalize( a->size() + b->size() );
        }

        bpos = a->pos() + a2b;
    }
    else if ( playmode != PM_PlayOn
              && b->getObjectType() == PObject::OT_BALL )
    {
        ...
        if ( b->pos() == a->pos() )
        {
            ...
            b2a = PVector::fromPolar( a->size() + b->size(), a_ang );
        }
        else
        {
            b2a = a->pos() - b->pos();
            b2a.normalize( a->size() + b->size() );
        }
    ...
    else
    {
             ...
             )
        {
            ...
            mid2a = PVector::fromPolar( ( a->size() + b->size() ) / 2.0 , a_ang );
            mid2b = PVector::fromPolar( ( a->size() + b->size() ) / 2.0 , b_ang );
        }
        else
        {
            mid2a.normalize( ( a->size() + b->size() ) / 2.0 );
            mid2b.normalize( ( a->size() + b->size() ) / 2.0 );
        }
      ...
      M_olcoach( static_cast< OnlineCoach * >( 0 ) )
{
    ...
    if ( ServerParam::instance().team_actuator_noise )
    {
        if ( side() == LEFT )
        {
            M_prand_factor_team     = ServerParam::instance().prand_factor_l;
            M_kick_rand_factor_team = ServerParam::instance().kick_rand_factor_l;
        }
        else
        {
            M_prand_factor_team     = ServerParam::instance().prand_factor_r;
            M_kick_rand_factor_team = ServerParam::instance().kick_rand_factor_r;
        }
    }
