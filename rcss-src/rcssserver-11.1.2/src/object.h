...
inline Value square( const Value & x ) { return x * x; }
...
//       {
//           return M_pos.vangle( obj.pos(), M_angle );
//       }
    ...
    //PObject::obj_type setObjectType ( const PObject::obj_type & object_type )
    //  {
...
// class PObjectTable {
// public:
//     typedef std::vector< const PObject * > Cont;
...
//     const
//     PObject * AssignObject( const PObject * obj );
...
//extern std::ostream& operator<< (std::ostream& o, const PObjectTable& tbl);

class MPObjectTable {
public:
    typedef std::vector< MPObject * > Cont;
...
private:
    ...
    MPObjectTable();
    const MPObjectTable & operator=( const MPObjectTable & );
public:

    MPObjectTable( Stadium & stadium );
    ~MPObjectTable();

    MPObject * append( MPObject* obj );

    void _inc( const PlayMode & playmode );
    void _turn();

    //! \todo move to Stadium and reimplement
    void collisions( const PlayMode & playmode );
...
//extern std::ostream& operator<< (std::ostream& o, const MPObjectTable& tbl);
