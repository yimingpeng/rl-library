...
class GridWorldMapper extends KeyboardMapper implements IntActionReceiver {
    ...
    private org.rlcommunity.rlglue.codec.types.Action getAction() {
        while (!nextActionSet) {
            try {
                ...
                Logger.getLogger(GridWorldMapper.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    ...
    protected void addActions(JComponent theComponent) {
        javax.swing.Action goLeft=new SimpleIntAction(1, this);
        javax.swing.Action goRight=new SimpleIntAction(0, this);
        javax.swing.Action goDown=new SimpleIntAction(2, this);
        javax.swing.Action goUp=new SimpleIntAction(3, this);
        
        theComponent.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_LEFT, 0), "goLeft");
        theComponent.getActionMap().put("goLeft", goLeft);
        theComponent.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT, 0), "goRight");
        theComponent.getActionMap().put("goRight", goRight);
        theComponent.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, 0), "goDown");
        theComponent.getActionMap().put("goDown", goDown);
        theComponent.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_UP, 0), "goUp");
        theComponent.getActionMap().put("goUp", goUp);
    }
}
