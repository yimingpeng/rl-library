...
namespace {
  ...
  const double modelbreadth = 0.04;
...
TaskRef get_task_hierarchy(taskspec_t *ts)
{
  ...
  for (int i = firstaction; i < firstaction + 4; ++i) {
    ...
    TaskRef task =
      PrimitiveTask::create(udrl[i - firstaction], i, explorationthreshold,
  ...
  for (int i = 0; i < numgoals; ++i) {
    ...
    TaskRef subgoal_primitive =
      PrimitiveTask::create(name, i + firstaction + 5, explorationthreshold,
